...
public abstract class Product {
	...
	//Made a subclass that includes the Sale and Lease Class
	public abstract class Agreements extends Product {
	public static class Lease extends Agreements {
		...
		//Having issues doing calculations due to differences in types
		//example: String * double.
		public double getTax() {
			double tax;
			if(Customer.getType() == "L") {
			tax = 0;
			} else if (Customer.getType() == "G") {
			tax = monthlyCost * 0.06;
			}
		...
		public void computeGrandTotal() {
			...
			double total;
			if(Customer.getType() == "L") {
				total = monthlyCost - 1000 + 50.75;
			} else if(Customer.getType() == "G") {
				total = monthlyCost + getTax();
			}
		}
	...
	public static class Sale extends Agreements {
		...
		public double computeGrandTotal() {
				...
				double total;
				if(Customer.getType() == "L") {
					total = totalCost - 1000 + 50.75;
				} else if(Customer.getType() == "G") {
					total = totalCost + getTax();
				}
			}
		}
	...
	//Made a subclass that includes Parking and Amenity.
	public abstract class Services extends Product {
	public static class Parking extends Services {
	...
	public static class Amenity extends Services {
		private String cost;
